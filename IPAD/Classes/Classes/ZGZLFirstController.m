...
#import "ZGZLFirstController.h"
#import "ZGZLSecondController.h"
#import "MPTabBarController.h"


@implementation ZGZLFirstController
...
- (void)viewDidLoad {
	...
	titleLabel.text = @"组工资料";
	[titleView addSubview:titleLabel];
	[titleLabel release];
	
	
	resultArray = [[NSMutableArray alloc] initWithArray:[[SQLiteOptions sharedSQLiteOptions]queryWithSQL:@"SELECT * FROM zugongziliao where parent_id = -1"]];
	NSLog(@"%@",resultArray);
...
{
	ZGZLSecondController *zc = [[ZGZLSecondController alloc] initWithStyle:UITableViewStylePlain];
	zc.tit = [[[tableView cellForRowAtIndexPath:indexPath]textLabel]text];
	zc.condition = [[tableView cellForRowAtIndexPath:indexPath]tag];
	MPTabBarController *tabbarController = [delegate tabBarController];
	[tabbarController showDetailController:zc animated:YES];
	[zc release];
}
