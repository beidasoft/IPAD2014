...
- (void)viewDidLoad {
	...
	titleLabel.text = @"干部年统";
	[titleView addSubview:titleLabel];
	[titleLabel release];
	
	
	resultArray = [[NSMutableArray alloc] initWithArray:[[SQLiteOptions sharedSQLiteOptions]queryWithSQL:@"SELECT * FROM ganbuniantong where parent_id = -1"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath 
{
	...
	GBNTSecond *dc = [[GBNTSecond alloc] initWithStyle:UITableViewStylePlain];
	...
	NSLog(@"GBNTSecond====%d",dc.condition);
