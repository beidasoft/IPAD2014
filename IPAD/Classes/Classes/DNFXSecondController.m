...
@implementation DNFXSecondController
...
- (void)viewDidLoad {
    [super viewDidLoad];
	self.title = self.tit;
	NSString *sql = [NSString stringWithFormat:@"SELECT * FROM dangneifenxi where perant_id = %d",self.condition];
	resultArray = [[NSMutableArray alloc] initWithArray:[[SQLiteOptions sharedSQLiteOptions]queryWithSQL:sql]];
	
	NSLog(@"shu chu=====%d",self.condition);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	 ...
	 */
	if ([[tableView cellForRowAtIndexPath:indexPath]tag] < 12)
	{
	...
	else 
	{
	    ...
	    DNFXThird *sc = [[DNFXThird alloc] initWithStyle:UITableViewStylePlain];
