...
- (void)viewDidLoad {
	...
	titleLabel.text = @"人才分析";
	[titleView addSubview:titleLabel];
	[titleLabel release];
	
	
	resultArray = [[NSMutableArray alloc] initWithArray:[[SQLiteOptions sharedSQLiteOptions]queryWithSQL:@"SELECT * FROM rencaifenxi where parent_id = -1"]];
	NSLog(@"%@",resultArray);
...
{
	RCFXSecondController *rc = [[RCFXSecondController alloc] initWithStyle:UITableViewStylePlain];
	rc.tit = [[[tableView cellForRowAtIndexPath:indexPath]textLabel]text];
	rc.condition = [[tableView cellForRowAtIndexPath:indexPath]tag];
	MPTabBarController *tabbarController = [delegate tabBarController];
	[tabbarController showDetailController:rc animated:YES];
	[rc release];
}
