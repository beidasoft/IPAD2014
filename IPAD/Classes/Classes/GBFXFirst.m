...
- (void)viewDidLoad {
	...
	titleLabel.text = @"干部分析";
	[titleView addSubview:titleLabel];
	[titleLabel release];
	
	
	resultArray = [[NSMutableArray alloc] initWithArray:[[SQLiteOptions sharedSQLiteOptions]queryWithSQL:@"SELECT * FROM ganbuceping where parent_id = -1"]];
	//NSLog(@"%@",resultArray);
...
{
	GBFXSecond *dc = [[GBFXSecond alloc] initWithStyle:UITableViewStylePlain];
